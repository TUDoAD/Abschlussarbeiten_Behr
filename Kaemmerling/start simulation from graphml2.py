...
def startsimulationfromgraphml(graph, inlet_temperature,inlet_pressure, compoundscompoundflow):
    ...
    for node in nodes:
        ...
        if graph._node[node]['node_class'] == 'Column' == node_class:
            ...
            group= {node:dict1}
            nx.set_node_attributes(graph,group)
            compounds = UnitOperation._node['unitoperation']['compounds'] 
            dict1 = {'compounds':compounds}
            group= {node:dict1}
            nx.set_node_attributes(graph,group)
        ...
        if graph._node[node]['node_class'] =='Column' != node_class and node in successors == True and 'simulated_node' not in graph._node[node] == True:  # start next function only if it was not run before (dict condition) and if it is in the successor list
            ...
            # get data from simulation and save it in graphml
            dict2 = {}
            list2 = list(DWSIMfunctions.m2.GetOverallComposition)
            ...
            nx.set_node_attributes(graph,group)
            dict2 = {}
            list2 = list(DWSIMfunctions.m3.GetOverallComposition)
            for key in compoundscompoundflow:
                for value in list2:
                    dict2.append(key, value)
