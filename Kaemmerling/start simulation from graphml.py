#apparate auslesen und hinzufügen
import os
...
number_of_mass_flows = (2 + nx.number_of_edges(f))
l = number_of_mass_flows * ['none']
...
for node in nodes:
    node_dict = f._node[node]
    
for c in l:  
    index = l.index(c)
    if index <= number_of_mass_flows:
        global('m'+ str(c)) = sim.AddObject(ObjectType.MaterialStream, 0, 50, 'dest')
...
inlet_attributes = nodes[0]
inlet_temperature = nx.get_node_attributes(f, 'inlet_temperature')
inlet_pressure = nx.get_node_attributes(f, 'inlet_pressure')
inlet_mass_flow = nx.get_node_attributes(f, 'inlet_mass_flow')
inlet_composition = nx.get_node_attributes(f, 'inlet_composition')
...
#compounds auslesen und hinzufügen
#input werte aus der 1.node auslesen
#gelesene werte der simulation übergeben
#property package hinzufügen
#apparatespezifische daten übergeben
#simulation starten
