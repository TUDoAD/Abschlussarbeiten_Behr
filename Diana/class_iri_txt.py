...
def create_list_IRIs(class_list, IRI_json_filename='iriDictionary'):
        ...
        for entity in class_list:
            match_dict = search_value_in_nested_dict(
                entity, onto_dict, match_dict)
...
def search_value_in_nested_dict(value, onto_dict, match_dict):
    # diese zeile kann dann weg wenn nur eine Ontologie betrachtet wird.
    for k in onto_dict.keys():
        # hier muss dann k rausgenommen werden. abhängig davon wie wein Dictionary aussieht
