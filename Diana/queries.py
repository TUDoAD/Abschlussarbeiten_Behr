...
def get_entities(doi):
    ...
    return sup_all, cat_all, reactant_all, product_all

def scopus_seach_process(doi, onto_pub_list ):
    queries=[]
    if doi!=None:
        sup_all, cat_all, reactant_all, product_all= get_entities(doi)

        df_all,_=ScopusSearchQueries(reac_all, sup_all, cat_all, reactant_all, product_all,queries)
    else:
        for p in onto_pub_list:
            sup_all, cat_all, reactant_all, product_all = get_entities(doi)
            list_reac_doi = get_reaction(reac=None,doi=doi) 
            reac_all = [*set([i[0].lower() for i in list_reac_doi])]
            df,queries = ScopusSearchQueries(reac_all, sup_all, cat_all, reactant_all, product_all,queries)
            df_all = pd.concat([df_all, df], axis=0, ignore_index=True)
    return df_all
    
def ScopusSearchQueries(reac_all, sup_all, cat_all, reactant_all, product_all,queries):
    columns= ['eid',
          'doi',
          'pii',
          'pubmed_id',
...
def reasoning_dois_onto(onto_name):
    ...
    print('The {} ontology is consist of following publications:'.format(onto_name))
