...
def call_subprocess(name, temperature, pressure, velocity, path):    
    ...
    for i in range(len(combinations)):
        ...
        name_sim = name_sim + f"_{combinations[i][0]}K_{combinations[i][1]}Pa_{combinations[i][2]}ms"
        temperature = str(combinations[i][0])
        pressure = str(combinations[i][1])
        velocity = str(combinations[i][2])
        command = ['python', 'SimulateMethanation.py', name_sim, temperature, pressure, velocity, new_dir, data_path]
